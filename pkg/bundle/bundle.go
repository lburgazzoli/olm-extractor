...
func Load(input string) (*manifests.Bundle, func(), error) {
...
func LoadFromImage(imageRef string) (*manifests.Bundle, func(), error) {
	...
	reg, err := containerdregistry.NewRegistry(containerdregistry.SkipTLSVerify(false))
	if err != nil {
		cleanup()
	...
	if err := reg.Pull(ctx, ref); err != nil {
		...
		return nil, nil, fmt.Errorf("failed to pull image %s: %w (check authentication if using a private registry)", imageRef, err)
	}
